...
- (UIPickerView *)addressPickerView{
    if (!_addressPickerView) {
        _addressPickerView = [[UIPickerView alloc]init];//WithFrame:
...
- (UIView *)titleBackgroundView{
    if (!_titleBackgroundView) {
        _titleBackgroundView = [[UIView alloc]init];//WithFrame:
...
- (UIButton *)cancelBtn{
    if (!_cancelBtn) {
        _cancelBtn = [[UIButton alloc]init];//WithFrame:
...
- (UIButton *)sureBtn{
    if (!_sureBtn) {
        _sureBtn = [[UIButton alloc]init];//WithFrame:
...
- (void)layoutSubviews
{
    ...
    [_addressPickerView makeConstraints:^(MASConstraintMaker *make) {
        make.left.and.bottom.and.right.equalTo(0);
        make.height.equalTo(200 * HEIGHTFIT);
    }];
    
    [_titleBackgroundView makeConstraints:^(MASConstraintMaker *make) {
        make.left.and.right.equalTo(0);
        make.bottom.equalTo(_addressPickerView.top).offset(0);
        make.height.equalTo(50 * HEIGHTFIT);
    }];
    
    [_cancelBtn makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_titleBackgroundView);
        make.left.equalTo(_titleBackgroundView);
        make.size.equalTo(CGSizeMake(75 * WIDTHFIT, 50 * HEIGHTFIT));
    }];
    
    [_sureBtn makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(_titleBackgroundView);
        make.right.equalTo(_titleBackgroundView);
        make.size.equalTo(CGSizeMake(75 * WIDTHFIT, 50 * HEIGHTFIT));
    }];
}
#pragma mark - 加载标题栏
