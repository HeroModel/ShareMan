...
@implementation PlutoCyclicCardLayout
- (instancetype)init
{
    self = [super init];
    if (self) {
        ...
        self.itemSize = CGSizeMake(200 * WIDTHFIT, 147.5 * HEIGHTFIT);
    }
    return self;
}
- (void)prepareLayout {
    ...
    offset = offset / 2.0;
    self.sectionInset = UIEdgeInsetsMake(0, offset, offset, offset);
    self.minimumInteritemSpacing = offset * 2;
}
...
- (NSArray<UICollectionViewLayoutAttributes *> *)layoutAttributesForElementsInRect:(CGRect)rect {
    ...
    for (UICollectionViewLayoutAttributes *attribute in layoutAttributes) {
        ...
        CGFloat normalizedDistance = fabs(distance / 500);
