...
- (void)viewDidLoad {
    ...
    switch (_series) {
        case 1:
            self.navigationItem.title = @"iPhone专区";
            break;
        case 2:
            self.navigationItem.title = @"OPPO专区";
            break;
        case 3:
            self.navigationItem.title = @"vivo专区";
            break;
        case 4:
            self.navigationItem.title = @"华为专区";
            break;
        default:
            break;
    }
...
//    CGFloat offset = 7.5f * WIDTHFIT;
//    CGFloat collectionViewHeight = 300.f;//100.0f;
...
//    [self.collectionView setContentOffset:CGPointMake(210 * WIDTHFIT * page, 0) animated:NO];
//}
- (void)upDataseriesWith:(int)series
{
    self.dataArray = [NSMutableArray new];
    NSString * string = [NSString stringWithFormat:@"brandId=%d", series];
    [NetTools POSTNetWithUrl:XUANZE body:string bodyStyle:NetBodySTRING resopnese:NetJSON hearder:nil success:^(id result) {
