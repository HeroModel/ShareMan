//
//  Share_YouHu_CollectionViewCell.m
//  ShareMan
//
...
@interface Share_YouHu_CollectionViewCell ()
...
/** <#注释#> */
@property(nonatomic, strong) UILabel * imei;
@end
@implementation Share_YouHu_CollectionViewCell
- (instancetype)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        [line makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_money.bottom).offset(10 * HEIGHTFIT);
            make.left.equalTo(0);
        ...
        UIButton * button = [UIButton ButtonWithType:UIButtonTypeCustom WithString:@"电子协议" WithBackgroundColor:UIColorFromRGB(GLOBAL_BACKGROUNDCOLOR_COLOR) WithTextColor:UIColorFromRGB(GLOBAL_PAGE_COLOR) WithFont:NAM_TITLE_FONT target:self action:@selector(didButtonPly:)];
        ...
        UILabel * label4 = [UILabel StringWithFont:NAM_TITLE WithTextColor:UIColorFromRGB(GLOBAL_SIGN_COLOR)];
        label4.textAlignment = NSTextAlignmentLeft;
        label4.text = @"手机串号";
        [backView addSubview:label4];
        [label4 makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(line4.bottom).offset(18 * HEIGHTFIT);
            make.left.equalTo(15 * WIDTHFIT);
            make.size.equalTo(CGSizeMake(80 * WIDTHFIT, 14 * HEIGHTFIT));
        }];
        
        self.imei = [UILabel StringWithFont:NAM_TITLE_B WithTextColor:UIColorFromRGB(GLOBAL_CONTEXT_COLOR)];
        self.imei.textAlignment = NSTextAlignmentRight;
        [backView addSubview:_imei];
        [_imei makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(line4.bottom).offset(18 * HEIGHTFIT);
            make.right.equalTo(-15 * WIDTHFIT);
            make.size.equalTo(CGSizeMake(200 * WIDTHFIT, 16 * HEIGHTFIT));
        }];
        
        UIView * line5 = [UIView lineWithView];
        [backView addSubview:line5];
        [line5 makeConstraints:^(MASConstraintMaker *make) {
            make.top.equalTo(_imei.bottom).offset(20 * WIDTHFIT);
            make.left.equalTo(15 * WIDTHFIT);
            make.right.equalTo(-15 * WIDTHFIT);
            make.height.equalTo(0.5 * HEIGHTFIT);
        }];
...
{
    if ([self.delegate respondsToSelector:@selector(order_id:)]) {
        [self.delegate order_id:_model.order_id];
    }
}
- (void)setModel:(OrderModel *)model
{
    ...
    self.date.text = model.sign_date_a;
    self.imei.text = model.imei;
}
@end
