//
//  ShareParticularsViewController.m
//  ShareMan
//
...
#import "OrderModel.h"
@interface ShareParticularsViewController ()
<
...
@implementation ShareParticularsViewController
{
...
- (void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    ...
    switch (index) {
        case 0:
        {
            [self upDateWithPhoneNumber:_phone WithToken:_token WithStatus:[NSString stringWithFormat:@"%ld", index]];
        }
            break;
        case 1:
        {
            [self upDateWithPhone1Number:_phone WithToken:_token WithStatus:[NSString stringWithFormat:@"%ld", index]];
        }
            break;
        case 2:
        {
            [self upDateWithPhone2Number:_phone WithToken:_token WithStatus:[NSString stringWithFormat:@"%ld", index]];
        }
            break;
        default:
            break;
...
- (void)dismissID:(NSString *)orderId
{
    ...
    [NetTools POSTNetWithUrl:ORDERDISMISS body:orderID bodyStyle:NetBodySTRING resopnese:NetJSON hearder:nil success:^(id result) {
        if ([result[@"code"] isEqualToString:@"0"]) {
            ...
            [self upDateWithPhoneNumber:_phone WithToken:_token WithStatus:@"1"];
        }
